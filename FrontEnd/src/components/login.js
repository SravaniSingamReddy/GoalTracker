            ...
            const userdetails=response.userDetails;
             localStorage.setItem("token", response.jwt);
                if(userdetails.role==="Super Admin"){
                    navigate("/superadmin",{
                        state: { id: userdetails.id ,userdetails:userdetails},
                      })
                }
                else if(userdetails.role==="Admin"){
                    navigate("/admin", {
                        state: { id: userdetails.id,gdo: userdetails.gdo ,userdetails:userdetails},
                      })
                }
                else{
                    navigate("/employee",{
                        state: { id: userdetails.id,check:false ,userdetails:userdetails},
                      })
                }
            }}}/>
            </div>
